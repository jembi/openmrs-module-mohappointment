...
<script type="text/javascript">
	...
	function showEditWindow(pId){
		$j("#divWindow").html("<div id='dialog' style='font-size: 0.9em;' title='<spring:message code='mohappointment.appointment.service.provider.form'/>'><div id='result'>"+$j('#editWindow').html()+"</div></div>");
		$j("#dialog").dialog({
			zIndex: 980,
			bgiframe: true,
			height: 220,
		...
		<table class="list_data">
			...
			<c:forEach items="${serviceProviders}" var="sp" varStatus="status">
				<tr>
					...
					<td class="rowValue ${status.count%2!=0?'even':''}"><input onclick="showEditDialog('${sp.serviceProviderId}');" type="button" id="btEdit" value="<spring:message code='mohappointment.general.edit'/>"></td>			
				</tr>
			</c:forEach>
		</table>
